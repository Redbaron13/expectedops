...
Modified to handle forced, validated updates across all databases.
Corrected sqlite3.Row access.
Integrated run counter and timestamp updates via GconfigEM.
Updated get_db_stats.
"""
import sqlite3
import os
import logging
...
import hashlib
import uuid
import GconfigEM # Use config to get DB names and manage state
...
def check_duplicate_by_hash(cursor, data_hash):
    """Checks if an opinion with the same DataHash already exists."""
    cursor.execute("SELECT 1 FROM opinions WHERE DataHash = ?", (data_hash,))
    return cursor.fetchone() is not None

# --- Main Data Insertion/Update Logic ---
def add_or_update_opinion_to_db(db_filename, opinion_data, is_validated, run_type):
    """
    Adds a new opinion record or updates an existing one based on UniqueID (derived from DataHash).
    Handles regular updates and forced validated overwrites.
    Returns status: "inserted", "updated_force_validated", "updated_metadata", "skipped_no_changes", "error_..."
    """
    ...
    try:
        ...
        if existing_opinion is None:
            ...
            log.info(f"Found existing opinion (UniqueID: {unique_id[:8]}...) for {app_docket_id} in {db_filename}. Comparing.")
            update_fields = {}
            force_overwrite = (run_type == 'manual-immediate' and is_validated)

            if force_overwrite:
                log.info(f"Forced validated run: Overwriting all fields for {unique_id[:8]}...")
                update_fields = {
                    "AppDocketID": opinion_data.get("AppDocketID"),
                    "ReleaseDate": opinion_data.get("ReleaseDate"),
                    "DuplicateFlag": duplicate_flag,
                    "LinkedDocketIDs": opinion_data.get("LinkedDocketIDs", ""),
                    "CaseName": opinion_data.get("CaseName"),
                    "LCdocketID": opinion_data.get("LCdocketID", ""),
                    "LCCounty": opinion_data.get("LCCounty", ""),
                    "Venue": opinion_data.get("Venue"),
                    "LowerCourtVenue": opinion_data.get("LowerCourtVenue"),
                    "LowerCourtSubCaseType": opinion_data.get("LowerCourtSubCaseType"),
                    "OPJURISAPP": opinion_data.get("OPJURISAPP", ""),
                    "DecisionTypeCode": opinion_data.get("DecisionTypeCode"),
                    "DecisionTypeText": opinion_data.get("DecisionTypeText"),
                    "StateAgency1": opinion_data.get("StateAgency1"),
                    "StateAgency2": opinion_data.get("StateAgency2"),
                    "CaseNotes": opinion_data.get("CaseNotes", ""),
                    "RunType": run_type,
                    "validated": True,
                    "last_validated_run_ts": now_ts
                }
                status_on_success = "updated_force_validated"
            else:
                # --- Standard Metadata Update Logic ---
                ...
                fields_to_compare = [
                    "StateAgency1", "StateAgency2", "LowerCourtVenue",
                    "LowerCourtSubCaseType", "OPJURISAPP", "LinkedDocketIDs",
                    "LCdocketID", "LCCounty", "Venue", "DecisionTypeCode",
                    "DecisionTypeText", "CaseNotes"
                ]
                for field in fields_to_compare:
                     new_value = opinion_data.get(field)
                     existing_value = existing_opinion[field] # Use dictionary access
                     if new_value != existing_value:
                          update_fields[field] = new_value
                          log.debug(f"Updating field '{field}' for {unique_id[:8]}...")

                status_on_success = "updated_metadata"

            # --- Execute Update if Changes Exist ---
            if update_fields:
                ...
                log.info(f"Skipping update for {unique_id[:8]}... in {db_filename}, no changes needed ({'forced run' if force_overwrite else 'standard run'}).")
...
def save_opinions_to_dbs(opinion_list, is_validated, run_type):
    """
    Saves a list of opinions to the appropriate databases based on run_type.
    Also increments run counter and updates timestamps.
    """
    db_files = GconfigEM.get_db_filenames()
    results = {}
    dbs_to_update = [] # List of tuples: (db_key, db_filename)
    db_keys_targeted = []

    if run_type == 'manual-immediate':
        log.info(f"Forced run detected. Targeting all configured databases.")
        db_keys_targeted = ["primary", "backup", "all_runs", "test"]
    else:
        # Standard run (typically scheduled primary) - target primary and all_runs
        log.info(f"Standard run type '{run_type}'. Targeting primary and all_runs databases.")
        db_keys_targeted = ["primary", "all_runs"]

    # Prepare results dict and dbs_to_update list
    for db_key in db_keys_targeted:
        db_filename = db_files.get(db_key)
        if db_filename:
            dbs_to_update.append((db_key, db_filename))
            results[db_key] = {"total": 0, "inserted": 0, "updated": 0, "skipped": 0, "error": 0}
        else:
            log.error(f"Database filename for '{db_key}' not configured. Skipping.")

    if not dbs_to_update:
        log.error("No target databases configured or found. Cannot save opinions.")
        return results

    successful_writes = {db_key: False for db_key, _ in dbs_to_update}
    overall_success = False # Track if at least one write happens
    ...
    # Log summary for each targeted database
    ...
    # Update config state only if at least one operation was attempted
    if any(r['total'] > 0 for r in results.values()):
        try:
            # Increment run counter once per call to this function if successful writes happened
            if overall_success:
                GconfigEM.increment_run_counter()
            # Update timestamps for databases that had successful writes
            for db_key, success in successful_writes.items():
                if success:
                    GconfigEM.update_last_run_timestamp(db_key)
        except Exception as e:
            log.error(f"Failed to update config (counter/timestamps): {e}", exc_info=True)


    return results

# --- Function to Get Previous Run Data ---
...
def get_db_stats(db_filename):
    """Provides actual statistics for a given database file."""
    ...
    try:
        ...
        # Calculate unvalidated
        stats["unvalidated"] = stats["total"] - stats["validated"]
    except sqlite3.Error as e:
        log.error(f"Database error getting stats from '{db_filename}': {e}", exc_info=True)
        stats["error"] = str(e)
    except ConnectionError as e:
        stats["error"] = str(e)
        # Already logged in get_db_connection
    except Exception as e:
